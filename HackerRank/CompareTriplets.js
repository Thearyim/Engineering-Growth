...
//   The return array is [2,1].
  ...
  class Solution {
    ...
    static List<int> compareTriplets(List<int> a, List<int> b) {
        // a[0]=1, b[0]=10 -> 10>1
        int firstPersonRating;
        int secondPersonRating;
        List<int> scores = new List<int>{ 0, 0 };
        for (int i = 0; i < a.Count; i++){
            firstPersonRating = a[i];
            secondPersonRating = b[i];
            if(firstPersonRating > secondPersonRating){
                scores[0]++;
            } else if (secondPersonRating > firstPersonRating) {
                scores[1]++;
            }
        }
        return scores;
